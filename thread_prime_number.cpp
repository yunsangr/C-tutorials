...
{
  // Get time before code starts executing
  int startTimeWithThreads = clock();
  FindPrimesWithThreads(1, 150000, 5);
  int endTimeWithThreads= clock();

  // Print out the number of seconds
  std::cout << "Execution Time with Threads : " <<
            (endTimeWithThreads - startTimeWithThreads)/double(CLOCKS_PER_SEC)
            << std::endl;
